...
def dfs(problema: LightsOut, contador: Contador = None):
    ...
    while fronteira:
        no_atual = fronteira.pop()  #POP (retira do topo)
        contador.soma()
        ...
        for acao in problema.acoes(no_atual.estado):
            ...
            fronteira.append(filho)  # Empilhamento
...
if __name__ == "__main__":
    ...
    if solucao:
        ...
        print(f"Ações realizadas entre linha e coluna {acoes}") #fica muito grande a representação 
        print(f"Tempo de execução : {time.time()-t:.5f}")
